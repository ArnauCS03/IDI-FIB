...
void MyGLWidget::initializeGL() {
  ...
  posFocusLoc = glGetUniformLocation (program->programId(), "posFocusFS");
...
void MyGLWidget::iniFocus() {
  ...
  focus_escena = true;
  posFocusEscena = View * glm::vec4(1, 1, 1, 1); // Inicialment es focus d'Escena
  ...
  switch (event->key()) {
    ...
    case Qt::Key_F:
      ...
      if (focus_escena) {  // passem focus de camara
        ...
        posFocusEscena = View * glm::vec4(1, 1, 1, 1);  
        glUniform4fv (posFocusLoc, 1, &posFocusEscena[0]);
        focus_escena = true;
      }
      break;
